...
async function handleCreateRace() {
	try {
		...
		console.error(`Error handling createRace ${error}`);
	}
}
	...
	return new Promise(resolve => {
		...
		const raceInterval = setInterval(async () => {
			try {
				...
				console.error(`Error running race ${error.message}`);
			}
		}, 500);
	})
}
...
async function handleAccelerate() {
	...
	try {
		...
		console.error(`Error handling acceleration ${error.message}`);
	}
}
...
function accelerate(id) {
	...
	try {
		...
		console.error(`Error accelerating ${error.message}`);
	}
}
